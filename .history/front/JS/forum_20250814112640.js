...
async function carregarPostagens() {
    ...
    const response = await fetch(`${API_BASE_URL}/postagens`);
        ...
        if (title) {
            ...
            // Adicionar botão para nova postagem se estiver logado (com base no usuário)
            const user = JSON.parse(localStorage.getItem('user') || 'null');
            if (user) {
...
async function verificarCurtida(postId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/likes/check/${postId}?userId=${user.id}`);
        const data = await response.json();
        
        if (response.ok && data.curtiu) {
...
async function adicionarComentario(postId) {
    ...
    const response = await fetch(`${API_BASE_URL}/comments`, {
            ...
            body: JSON.stringify({ usuario_id: user.id, postagem_id: postId, texto })
        });
        const data = await response.json();
        if (response.ok) {
            textArea.value = '';
